...
function mapAccountInputToQBO(input) {
    const qbo = {};
    // Commonly used Account fields
    if (input.name)
        qbo.Name = input.name;
    if (input.fullyQualifiedName)
        qbo.FullyQualifiedName = input.fullyQualifiedName;
    if (input.accountType)
        qbo.AccountType = input.accountType; // e.g., "Bank", "Accounts Receivable", ...
    if (input.accountSubType)
        qbo.AccountSubType = input.accountSubType; // e.g., "Checking", "AccountsReceivable", ...
    if (input.description)
        qbo.Description = input.description;
    if (input.classification)
        qbo.Classification = input.classification; // e.g., "Asset", "Liability", "Equity", "Income", "Expense"
    if (input.accountNumber)
        qbo.AcctNum = input.accountNumber;
    if (typeof input.taxCodeRef === "string")
        qbo.TaxCodeRef = { value: input.taxCodeRef };
    if (input.currencyRef)
        qbo.CurrencyRef = { value: input.currencyRef }; // e.g., "USD"
    if (typeof input.subAccount === "boolean")
        qbo.SubAccount = input.subAccount;
    if (input.parentRef)
        qbo.ParentRef = { value: input.parentRef }; // parent Account.Id
    if (typeof input.currentBalance === "number")
        qbo.CurrentBalance = input.currentBalance; // usually read-only; avoid on create
    if (typeof input.active === "boolean")
        qbo.Active = input.active;
    return qbo;
}
// --- Zod Schemas ---
...
async function main() {
    ...
    server.tool("get_account_by_id", "Fetch a QuickBooks account by ID", { accountId: z.string().describe("The QuickBooks Account ID") }, async ({ accountId }) => {
        const data = await qbRequest(`account/${accountId}`, { method: "GET" });
        const account = data?.Account ?? data;
        return { content: [{ type: "text", text: JSON.stringify(account, null, 2) }] };
    });
    // List accounts (paged)
    server.tool("list_accounts", "List accounts with pagination (uses QBO query endpoint)", accountPaginationSchema.shape, async ({ startPosition, maxResults }) => {
    ...
    // Search accounts
    server.tool("search_accounts", "Search accounts by name/type/subtype/classification with optional pagination", accountSearchSchema.shape, async ({ name, accountType, accountSubType, classification, activeOnly, startPosition, maxResults, orderBy, sort }) => {
        const parts = [];
        const esc = (s) => s.replace(/'/g, "\\'");
        if (typeof activeOnly === "boolean")
            parts.push(`Active = ${activeOnly ? "true" : "false"}`);
        if (name)
            parts.push(`Name LIKE '${esc(name)}%'`);
        if (accountType)
            parts.push(`AccountType = '${esc(accountType)}'`);
        if (accountSubType)
            parts.push(`AccountSubType = '${esc(accountSubType)}'`);
        if (classification)
            parts.push(`Classification = '${esc(classification)}'`);
    ...
    // Create account
    server.tool("create_account", "Create a new QuickBooks account", accountCreateSchema.shape, async (input) => {
    ...
    server.tool("update_account", "Update an existing QuickBooks account (sparse update by default)", accountUpdateSchema.shape, async (input) => {
        const { accountId, sparse = true, ...patch } = input;
        // get latest SyncToken
    ...
    server.tool("set_account_active", "Activate or deactivate an account (Active=true/false)", {
    ...
    console.error("QuickBooks MCP server running");
}
main().catch((err) => {
